...
<p class="note">${ c.event_pricing_disclaimer }</p>
...
%         if category.name == "Ticket" and is_speaker:
          <p>This registration fee will be waived upon request, by e-mailing
          <a href="mailto:program@pycon-au.org"><tt>program@pycon-au.org</tt></a>. 
          Paid speaker registrations will help to boost our financial assistance programme.
          </p>
%         endif
...
%         if len(category.products) == 0 or (len(category.products) == 1 and category.products[0].cost == 0):
          ...
          for information. You <strong>must</strong> book
...
% endfor
        ...
        <fieldset>
          ...
          <p class="note">Please enter any requirements if necessary; access requirements, etc. Our venue is classed as accessable, but our current dinner plans include travel. Please leave blank if you have no special requirements. Joke entries are not appreciated, this is a serious question.</p>
...
% if is_speaker:
          <fieldset>
            ...
            <p>As a service to the Python community,
            PyCon Australia would like to make your presentation available to the public.
            This involves video­taping your talk, and offering the video/audio and slides.</p>

            <p>If you have allowed PyCon Australia to publish your slides, there will
