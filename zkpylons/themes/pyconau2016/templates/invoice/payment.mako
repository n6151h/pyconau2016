...
${ h.hidden('payment_reference', c.event_shortname + ' i-' + str(c.payment.invoice.id) + ' p-' + str(c.payment.id)) }
${ h.hidden('vendor_name', 'linuxprojects') }
${ h.hidden('cards_accepted', 'VISA,MASTERCARD,AMEX') }
${ h.hidden('gst_rate', '10') }
${ h.hidden('gst_added', 'TRUE') }
${ h.hidden(c.event_name + ' Registration', h.integer_to_currency(c.payment.amount, unit='' )) }
${ h.hidden('hidden_fields', 'submit') }
${ h.hidden('receipt_address', c.payment.invoice.person.email_address) }
${ h.hidden('return_link_text', 'Return to ' + c.event_name) }
${ h.hidden('return_link_url', h.url_for(qualified=True, controller='invoice', action='view', id=c.payment.invoice.id)) }
${ h.hidden('reply_link_url', h.url_for(protocol='http', qualified=True, controller='payment', action='new', id=c.payment.id, hash=None, payment_id=c.payment.id, invoice_id=c.payment.invoice_id, summary_code='', response_amount='', currency='', card_name='', card_type='', card_number='', card_expiry='', card_mac='', response_code='', bank_reference='', response_text='', remote_ip='', receipt_address='')) }
